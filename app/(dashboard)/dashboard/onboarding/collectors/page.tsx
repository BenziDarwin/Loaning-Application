...
export default function OnboardingCollectorsPage() {
  ...
  const handleCreateCollector = (collector: Omit<Collector, "id" | "enrollmentDate" | "password">) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Collector Onboarding</h2>
          <p className="text-muted-foreground">Register new collectors and manage their information</p>
        </div>
          ...
          <TableBody>
            {collectors.map((collector) => (
              <TableRow key={collector.id}>
                <TableCell>
                  {collector.firstName} {collector.secondName} {collector.thirdName}
                </TableCell>
                <TableCell>{collector.idNumber}</TableCell>
                <TableCell>{collector.phoneNumber}</TableCell>
                <TableCell>{collector.email || "N/A"}</TableCell>
                <TableCell>{collector.enrollmentDate.toLocaleDateString()}</TableCell>
